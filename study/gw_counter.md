...
| 指标 | 旧版（逐字节单趟） | 优化后（双趟宽累加） | 变化 |
|------|:---:|:---:|:---:|
| 实际 TPS | 9327.7 | 9736.3 | — |
| 样本数 | 46640 | 48682 | — |
| **平均延迟** | 442.9ns | **359.4ns** | **↓18.9%** |
| P50 | 241ns | 201ns | ↓16.6% |
| P75 | 300ns | 240ns | ↓20.0% |
| P90 | 441ns | 340ns | **↓22.9%** |
| 失败率 | 0% | 0% | 持平 |
...
- **原因**：高 TPS 下每笔委托的固定开销（序列化 + 校验和）占比更高，双趟宽累加 + memcpy 整块的优势更突出。
- **功能正确性**：委托/撤单/成交回报全部通过，校验和匹配，0 失败。
- **关键结论**：在 10000TPS 高负载下，序列化优化带来显著收益（~20%）。但 2000TPS 下 P50 仅 430-441ns，与 GOne(fpga) 的 120ns 相比，剩余差距主要来自 `GwSessionCache` 查找（string+hash）。
- **下一步**：若需在低 TPS 下进一步逼近 fpga 的 ~200ns，需执行方案 1（会话迁 counter 成员，消除 string+hash 查找）。