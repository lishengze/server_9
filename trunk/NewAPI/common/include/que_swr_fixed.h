...
/**
 * @file que_swr_fixed.h
 * @brief 单写单读固定大小队列模块
 *
 * 提供一个高性能的单进程单写单读队列实现，专门用于存储POD类型数据。
 * 队列采用环形缓冲区设计，支持原子操作保证线程安全。
 *
 * 主要特性：
 * - 单写单读模式，无锁设计
 * - 固定大小数据项，支持POD类型
 ...
 * @note 支持内存对齐以提高性能
 */
template <class T> class alignas(CACHE_ALIGN_SIZE) que_swr_fixed {
protected:
  /**
   * @brief 队列项类型定义
   */
...
public:
   ...
   * @return T* 数据指针
   */
  FORCE_INLINE T *get_data(int64 pos) { return &(pbuf[pos & mask].data); }
   ...
   */
  FORCE_INLINE int64 read_get(T *&o_data) {
    int64 pos = rdcmt;
   ...
   */
  FORCE_INLINE int64 read_get(T *&o_data, int64 read_pos) {
   ...
   */
  int32 find_copy(T *o_buf, int64 pos) {
   ...
   * @return int64 当前写位置
   */
  FORCE_INLINE int64 get_write_pos() { return atomic_load64(&wrcmt); }
   ...
   * @return int64 当前读位置
   */
  FORCE_INLINE int64 get_read_pos() { return atomic_load64(&rdcmt); }
