// tcpdirect_engine - TCPDirect 极速引擎实现
//
// 与 single_socket_engine 结构相同, 但使用 tcpdir_link (Solarflare TCPDirect)
// 仅在 tcpdirect 模式下使用 (C2/C4 配置)
...
int32 tcpdirect_engine<TFastCounter>::init(const api_config_impl &cfg, TFastCounter *tfst, lb_common::lb_log *log) {
  ...
  const char *solarflare_iface = cfg.get_solarflare_iface();
  ...
  if (ret < 0) {
    error_log(tlh) << "init tcpdirect engine queue error,que_size=" << tq_size << ",ret=" << ret << end_log;
    return LBAPI_ERR_QUEUE_INIT;
  }

  int32 once_recv_len = sizeof(trade_rtn) * 4;
  ret = link_.init(log, tfst, solarflare_iface, LINK_TYPE_SPEED_TRADE, once_recv_len, check_interval, max_fails);
  if (ret < 0) {
    return ret;
  }
  ...
  if (ret < 0) {
    error_log(tlh) << "init tcpdirect engine timer error,interval=" << check_interval << ",ret=" << ret << end_log;
    return LBAPI_ERR_TIMER_INIT;
  }

  ret = init_th(0, tcpuid, 0);
  if (ret < 0) {
    error_log(tlh) << "init tcpdirect engine thread error,cpuid=" << tcpuid << ",ret=" << ret << end_log;
    return LBAPI_ERR_THREAD_INIT;
  }
  info_log(tlh) << "init tcpdirect engine ok,cpuid=" << tcpuid << ",timer_interval=" << heart_interval
                << ",que_size=" << tq_size << ",once_recv_len=" << once_recv_len << end_log;
  return 0;
}

template <class TFastCounter> int32 tcpdirect_engine<TFastCounter>::start() {

  lb_common::lb_log_hand tlh(log_);

  int32 ret = run();
  if (ret < 0) {
    error_log(tlh) << "start tcpdirect engine thread error,ret=" << ret << end_log;
    return LBAPI_ERR_THRAD_START;
  }
  info_log(tlh) << "start tcpdirect engine ok" << end_log;
  return 0;
}
...
template <class TFastCounter> void tcpdirect_engine<TFastCounter>::stop() {
  ...
  lb_common::lb_log_hand tlh(log_);
  info_log(tlh) << "stop tcpdirect engine ok" << end_log;
}

template <class TFastCounter> void tcpdirect_engine<TFastCounter>::do_work() {
...
template <class TFastCounter> void tcpdirect_engine<TFastCounter>::deal_cust_login(const acc_login_event_info &pmlog) {
  lb_common::lb_log_hand tlh(log_);
  char log_buf[8192];

  // 只可能是个微柜台的登陆
  if (counter_->get_counter_type() != static_cast<int32_t>(counter_type::gw_direct)) {
    error_log(tlh) << "fast link only support gw counter user login,branch_id=" << pmlog.branch_id
  ...
  if (link_.is_free()) {
    ...
    if (ret < 0) {
      error_log(tlh) << "fast link connect to login error,branch_id=" << pmlog.branch_id
                     << ",fund_account=" << pmlog.fund_account_id << ",session=" << pmlog.session
  ...
  if (tlen < 0) {
    ...
    counter_->ans_cust_login(pmlog, LBAPI_ERR_BUILD_MSG, "build fast counter msg error");
    return;
  } else if (tlen == 0) {
  ...
  if (ret > 0) {
    info_log(tlh) << "fast link send cust login ok,branch_id=" << pmlog.branch_id
                  << ",fund_account=" << pmlog.fund_account_id << ",session=" << pmlog.session
                  << ",client_req_no=" << pmlog.cust_req_no << ",msglen=" << tlen << end_log;
  } else {
    error_log(tlh) << "fast link send cust login error,branch_id=" << pmlog.branch_id
                   << ",fund_account=" << pmlog.fund_account_id << ",session=" << pmlog.session
                   << ",client_req_no=" << pmlog.cust_req_no << ",ret=" << ret << end_log;
...
template <class TFastCounter>
void tcpdirect_engine<TFastCounter>::deal_fpga_core_connect(const fpga_core_connect_info &pmlog) {
  lb_common::lb_log_hand tlh(log_);
...
template <class TFastCounter> int32 tcpdirect_engine<TFastCounter>::add_timer_poll(lb_common::mthread *th) {
  lb_common::lb_log_hand tlh(log_);
  int32 ret = timer_op_.add_timer_poll(th);
  if (ret < 0) {
    error_log(tlh) << "tcpdirect link add timer error,ret=" << ret << end_log;
    return LBAPI_ERR_ADD_TIMER;
  }
  info_log(tlh) << "tcpdirect link add timer ok" << end_log;
  return 0;
}

template <class TFastCounter> void tcpdirect_engine<TFastCounter>::eng_link_op::deal_heart_msg_ans(int16 link_type) {
  owner_->link_.deal_heart_ans();
}
template <class TFastCounter>
void tcpdirect_engine<TFastCounter>::eng_link_op::deal_close_link(int16 link_type, int32 err_code) {
  owner_->link_.close_ch(err_code);
}

// 显式实例化
template class tcpdir_link<gw_counter_direct>;
template class tcpdir_link<fpga_counter_direct>;
//template class tcpdir_link<fpga_counter_gateway>;

template class tcpdirect_engine<gw_counter_direct>;
template class tcpdirect_engine<fpga_counter_direct>;
//template class tcpdirect_engine<fpga_counter_gateway>;

template class link_timer_op<tcpdir_link<gw_counter_direct>, tcpdirect_engine<gw_counter_direct>>;
template class link_timer_op<tcpdir_link<fpga_counter_direct>, tcpdirect_engine<fpga_counter_direct>>;
//template class link_timer_op<tcpdir_link<fpga_counter_gateway>, tcpdirect_engine<fpga_counter_gateway>>;
