...
public:
  ...
  /// 关闭链接
  void close_ch(int32 err = 0) { ch_.close_ch(err); }
  ...
  void deal_ch_error(aio_socket_ch_t *pch, int32 err_type, int32 err_code) override {
    ...
    error_log(tlh) << "get socket link error to close,link_type=" << link_type_
                   << ",active_link=" << active_idx_
                   << ",remote_ip=" << addrs_[active_idx_].ip
                   << ",remote_port=" << addrs_[active_idx_].port
  ...
  void deal_ch_closing(aio_socket_ch_t *pch, int32 errcode) override {
    lb_common::lb_log_hand tlh(log_);
    warning_log(tlh) << "socket link is closing,link_type=" << link_type_
                     << ",active_link=" << active_idx_
                     << ",remote_ip=" << addrs_[active_idx_].ip
  ...
  void deal_recv_stop(aio_socket_ch_t *pch) override {
    lb_common::lb_log_hand tlh(log_);
    info_log(tlh) << "socket link recv stop,link_type=" << link_type_
                  << ",active_link=" << active_idx_
                  << ",remote_ip=" << addrs_[active_idx_].ip
                  << ",remote_port=" << addrs_[active_idx_].port << end_log;
  }
  void deal_ch_closed(aio_socket_ch_t *pch, int32 errcode) override {
    ...
    lb_common::lb_log_hand tlh(log_);
    info_log(tlh) << "socket link closed end,link_type=" << link_type_
                  << ",active_link=" << active_idx_
                  << ",remote_ip=" << addrs_[active_idx_].ip
                  << ",remote_port=" << addrs_[active_idx_].port << end_log;
  }
  void deal_ch_connect(aio_socket_ch_t *pch, lb_common::csock_addr &localaddr) override {
    lb_common::lb_log_hand tlh(log_);
    info_log(tlh) << "socket link connected,link_type=" << link_type_
                  << ",active_link=" << active_idx_
                  << ",remote_ip=" << addrs_[active_idx_].ip
                  << ",remote_port=" << addrs_[active_idx_].port
...
private:
  ...
  void build_aio_attr_(lb_common::aio_attr &buf_attr, lb_common::channel_attr &ch_attr, int32 recv_pool_num) {
    ...
    buf_attr.onerecvtimes = recv_pool_num;
    buf_attr.oncerecvlen = once_recv_len_;
    buf_attr.maxmsglen = 1 * 1024 * 1024;
    buf_attr.buf_size = 2 * 1024 * 1024;
